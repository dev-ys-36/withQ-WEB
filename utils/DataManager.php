...
class DataManager{
	...
	public const CNF = DataManager::PROPERTIES; // .cnf
	...
	public const ENUMERATION = DataManager::ENUM;
	...
	/** @var int */
	private $type = DataManager::DETECT;
	/** @var int */
	...
	public static $formats = [
		"properties" => DataManager::PROPERTIES,
		"cnf" => DataManager::CNF,
		"conf" => DataManager::CNF,
		"config" => DataManager::CNF,
		"json" => DataManager::JSON,
		"js" => DataManager::JSON,
		"yml" => DataManager::YAML,
		"yaml" => DataManager::YAML,
		//"export" => DataManager::EXPORT,
		//"xport" => DataManager::EXPORT,
		"sl" => DataManager::SERIALIZED,
		"serialize" => DataManager::SERIALIZED,
		"txt" => DataManager::ENUM,
		"list" => DataManager::ENUM,
		"enum" => DataManager::ENUM
	];
	 ...
	 */
	public function __construct(string $file, int $type = DataManager::DETECT, array $default = [], &$correct = null){
	 ...
	 * @phpstan-param array<string, mixed> $default
	 */
	public function load(string $file, int $type = DataManager::DETECT, array $default = []) : bool{
		$this->correct = true;
		$this->file = $file;

		$this->type = $type;
		if($this->type === DataManager::DETECT){
			$extension = explode(".", basename($this->file));
			$extension = strtolower(trim(array_pop($extension)));
			if(isset(DataManager::$formats[$extension])){
				$this->type = DataManager::$formats[$extension];
			}else{
				$this->correct = false;
			}
		}
		...
		if(!file_exists($file)){
			...
			if($this->correct){
				...
				$config = null;
				switch($this->type){
					case DataManager::PROPERTIES:
						$config = $this->parseProperties($content);
						break;
					case DataManager::JSON:
						$config = json_decode($content, true);
						break;
					case DataManager::YAML:
						$content = self::fixYAMLIndexes($content);
						$config = yaml_parse($content);
						break;
					case DataManager::SERIALIZED:
						$config = unserialize($content);
						break;
					case DataManager::ENUM:
